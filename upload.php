...
if ($_FILES['image']['error'] == 0) {
    ...
    $original_name = preg_replace('/[^a-zA-Z0-9_-]/', '_', $original_name); // Özel karakterleri temizle
    $original_name = substr($original_name, 0, 50); // Maksimum 50 karakter

    // Tarih ve benzersiz ID ile dosya adı oluştur
    $date_prefix = date('Y-m-d_H-i-s');
    $unique_id = uniqid();
    $file_name = $date_prefix . '_' . $original_name . '_' . $unique_id . '.' . $file_extension;
    $target_file = $target_dir . $file_name;

    // Dosya zaten var mı kontrol et (ekstra güvenlik)
    $counter = 1;
    while (file_exists($target_file)) {
        $file_name = $date_prefix . '_' . $original_name . '_' . $unique_id . '_' . $counter . '.' . $file_extension;
        $target_file = $target_dir . $file_name;
        $counter++;
    }
